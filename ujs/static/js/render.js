...
function renderTableBlock(json) {
  try {
    ...
    const th = t.headers.map(h =>
      `<th class="text-left text-court-dim font-medium px-2 py-1.5 border-b border-navy-border bg-navy/50 text-[13px] whitespace-nowrap">${esc2(h)}</th>`
    ).join('');
    const rows = t.rows.map(r => '<tr>' + r.map(c =>
      `<td class="text-left text-court-text px-2 py-1 border-b border-navy-border/50 text-[13px]">${esc2(String(c))}</td>`
    ).join('') + '</tr>').join('');
...
function renderMdTable(block) {
  ...
  let out = '<div class="overflow-x-auto my-3"><table class="w-full text-[13px] border-collapse">';
...
function renderMd(text) {
  ...
  html = html.replace(/^### (.+)$/gm, '<h4 class="text-white font-semibold text-[16px] mt-4 mb-1">$1</h4>');
  html = html.replace(/^## (.+)$/gm, '<h3 class="text-white font-semibold text-[17px] mt-5 mb-1.5 pb-1 border-b border-navy-border">$1</h3>');
  html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
  html = html.replace(/\*{1,2}Source:\s*(.+?)\*{0,2}$/gm, '<div class="text-gold text-[13px] mt-2">Source: $1</div>');
  html = html.replace(/\*Searching for news coverage\.\.\.\*/g,
    '<div class="text-court-dim text-[14px] mt-3 flex items-center gap-2 animate-pulse">' +
    '<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>' +
    'Searching for news coverage...</div>');
  html = html.replace(/\*(.+?)\*/g, '<em>$1</em>');
  html = html.replace(/^- (.+)$/gm, '<div class="pl-4 py-[1px] text-[15px] before:content-[\'•\'] before:text-gold before:mr-2">$1</div>');
  html = html.replace(/^\d+\. (.+)$/gm, '<div class="pl-4 py-[1px] text-[15px]">$1</div>');
  html = html.replace(/\n\n/g, '</p><p class="mt-2">');
  html = html.replace(/\n/g, '<br>');
  html = '<p>' + html + '</p>';
  ...
  html = html.replace(/\u2603BLOCK(\d+)\u2603/g, function(_, idx) {
    ...
    if (b.type === 'chart') return `<div data-chart="${esc2(b.content).replace(/"/g,'&quot;').replace(/'/g,'&#39;')}" class="my-3 py-2 text-[12px] text-court-dim">Loading chart...</div>`;
    return `<pre class="bg-navy/50 rounded p-2 text-[13px] my-2 overflow-x-auto"><code>${esc2(b.content)}</code></pre>`;
  });
