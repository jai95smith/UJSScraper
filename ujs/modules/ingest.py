...
def process_queue(batch_size=10):
    """Process pending items in the ingest queue."""
    processed = 0
    for _ in range(batch_size):
        ...
        print(f"[queue] Processing {docket_number} (job {job_id})")
        try:
...
def refresh_stale(active_hours=24, closed_days=7, batch_size=20):
    """Re-analyze stale dockets to detect changes."""
    with db.connect() as conn:
    ...
    print(f"[refresh] {len(stale)} stale dockets to re-analyze")
    refreshed = 0
    for row in stale:
...
def run_cycle(county=None, docket_type=None, lookback_days=1,
              lookahead_days=7, analyze_batch=10, refresh_batch=20):
    ...
    try:
        queued = process_queue(analyze_batch)
...
def main():
    ...
    if args.queue_only:
        ...
        print(f"Processed {processed} queue jobs")
    elif args.refresh_only:
        refreshed = refresh_stale()
        print(f"Refreshed {refreshed} dockets")
    elif args.once:
        run_cycle(**kwargs)
    else:
